...
//
// Note that this is v3.0 of roller.
//
...
// or manipulate the file that roller writes to will also result in improper
// behavior.
package roller
...
type Options struct {
	...
	// based on age.
	MaxAge time.Duration `toml:"max_age" comment:"MaxAge is the maximum time to retain old log files based on the timestamp encoded in their filename."`
...
//
// If the file exists and is less than maxSize bytes, roller will open and
...
func (r *Roller) oldLogFiles() ([]logInfo, error) {
	...
	for _, f := range files {
		...
		// by roller, and therefore it's not a backup file.
	}
...
func compressLogFile(src, dst string) (err error) {
	...
	defer func(f *os.File) {
		err := f.Close()
		if err != nil {
			log.Printf("Failed to close the file: %s", err)
		}
	}(f)
	...
	defer func(gzf *os.File) {
		err := gzf.Close()
		if err != nil {
			log.Printf("Failed to close the gzf: %s", err)
		}
	}(gzf)
